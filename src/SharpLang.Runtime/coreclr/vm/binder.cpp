...
#if !SHARPLANG_NOTIMPLEMENTED
    pMT = System_SharpLangModule__ResolveType_System_Byte__System_Byte__(GetModule(), d->nameSpace, d->name)->runtimeEEType;
#else
...
#if !defined(SHARPLANG_BINDER_NOTIMPLEMENTED)
    assert(false);
    return NULL;
...
#if !SHARPLANG_NOTIMPLEMENTED
    pFD = System_SharpLangEEType__FindField_System_Byte__(pMT, d->name);
#else
...
#if !SHARPLANG_NOTIMPLEMENTED
    m_pClasses = (MethodTable**)malloc(m_cClasses * sizeof(*m_pClasses));
    m_pMethods = (MethodDesc**)malloc(m_cMethods * sizeof(*m_pMethods));
    m_pFields = (FieldDesc**)malloc(m_cFields * sizeof(*m_pFields));
    ZeroMemory(m_pClasses, m_cClasses * sizeof(*m_pClasses));
    ZeroMemory(m_pMethods, m_cMethods * sizeof(*m_pMethods));
    ZeroMemory(m_pFields, m_cFields * sizeof(*m_pFields));
#else
