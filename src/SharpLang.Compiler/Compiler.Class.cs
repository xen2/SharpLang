        ...
        private void BuildRuntimeType(Class @class)
        {
            ...
            var staticFieldsInitializer = LLVM.ConstNamedStruct(runtimeTypeInfoTypeElements[(int)RuntimeTypeInfoFields.StaticFields], @class.StaticFields.Select(field =>
            {
                ...
                if ((field.Key.Attributes & FieldAttributes.HasFieldRVA) != 0)
                {
                    ...
                    if (LLVM.GetTypeKind(fieldType.DefaultType) == TypeKind.IntegerTypeKind)
                    {
                        unsafe
                        {
                            fixed (byte* initalValueStart = initialValue)
                            {
                                if (LLVM.GetIntTypeWidth(fieldType.DefaultType) == 64)
                                    return LLVM.ConstInt(fieldType.DefaultType, *(ulong*)initalValueStart, false);
                                if (LLVM.GetIntTypeWidth(fieldType.DefaultType) == 32)
                                    return LLVM.ConstInt(fieldType.DefaultType, *(uint*)initalValueStart, false);
                            }
                        }
                    }
                    ...
                    if (LLVM.GetTypeKind(fieldType.DefaultType) != TypeKind.ArrayTypeKind)
                ...
                return LLVM.ConstNull(fieldType.DefaultType);
            }).ToArray());

            // Get array element type
            var elementTypeRef = @class.Type.TypeReference is ArrayType ? ((ArrayType)@class.Type.TypeReference).ElementType : null;
            var elementType = elementTypeRef != null ? GetType(elementTypeRef, TypeState.StackComplete) : null;
            var elementTypeSize = elementType != null ? LLVM.ConstIntCast(LLVM.SizeOf(elementType.DefaultType), int32Type, false) : LLVM.ConstInt(int32Type, 0, false);
